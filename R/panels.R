...
yHelp = yTop  ; xHelp = "30px"
yPlot = "75px"; xPlot = "50px"
ySett = "80px"; xSett = "10px"
...
##> Plot Legends ----
plotHIV.legend  = shiny::HTML("<p><b>Figure. HIV subtype frequencies</b><br /> Plasma samples were obtained for a subset of 2079 ART-naïve, HIV+ participants from 21 countries enrolled in the START study. Extracted HIV RNA was sequenced using Illumina MiSeq for paired-end sequencing.</p>")
plotHLA.legend  = shiny::HTML("<p><b>Figure. Imputed HLA alleles</b><br /> Imputation of classic HLA class I alleles (HLA-A, HLA-B and HLA-C) for 2546 genotyped ART-naïve, HIV-infected participants was performed with HIBAG at 4-digit resolution. See main paper for full details: Briefly, a multi-ethnic pre-trained model was used for imputation with a minimum out-of-bag accuracy of over 90% for all loci. HLA diversity was measured by the inverse Simpson index based on the HLA allele frequencies per locus and country. This index represents the complement of the probability that two participants would have the same HLA allele for a selected locus in a country.</p>")
plotBIND.legend = shiny::HTML("<p><b>Figure. BIND Title</b><br /> Extracted HIV reads were fragmented into 27-mers using KAT and those with a count higher than 1 were translated into peptide sequences of 9 amino acids length to fit the mean length of HLA Class I epitopes. Peptides were mapped to ten major HIV proteins (Asp, Gag-Pol, Nef, Vpr, Vpu, gp160, Vif, Pr55, Rev, Tat) from NCBI RefSeq NC001802.1 using BLAST 2.8.1 (blastp-short) excluding hits with an E-value > 1E-05. To compare HIV peptidomes with random sequences, a set of half-million 9-mers were generated by processing the same number of random protein sequences from Uniprot. Binding affinities of 268 class I HLA alleles to HIV and random peptidomes were predicted using NetMHCpan 4.0. The binding threshold was set at 0.426, which when intersected with original of sample and corresponding imputed HLA types results in the 3 major groupings presented above: (Inactive) being those peptides that show no binding in locally imputed HLA types; (Exogenous) being those peptides that are not found in local HIV samples but that would be bound by locally imputed HLA types; and lastly (Endogenous) being those peptides found in local HIV samples and predicted to be bound by locally imputed HLA types.</p>")
...
plotPanel = pushbar::pushbar(
  ...
  shiny::conditionalPanel('input.showTREE',
    shiny::div(id="treeBIND",
      shiny::fluidRow(
        shiny::column(width = 4, shiny::tags$image(src="./tree_small.png", width = "100%")),
        shiny::column(width = 5,
          shiny::tags$p(shiny::h3("Functional consensus clustering of predicted binding affinities to HIV peptides"), shiny::p("Predicted binding affinities to 173,792 HIV peptides were used to calculate the HLA allele distances used for consensus clustering and represented as a dendrogram through hierarchical clustering. Associations to log10(HIV-VL) of each node (HLA functional node) and leaves (HLA alleles) in the dendrogram were tested and adjusted by sex, self-reported race, and country. Associations were defined by an adjusted p-value (Benjamini-Hochberg) < 0.05 and are represented as thick branches for nodes and black triangles for leaves. White triangles indicate HLA alleles detected in our cohort. The effect of the respective associations is colour-coded from protective effect (blue) to detrimental (red). On the outer ring, HLA allele counts are depicted as green bar plots. An interactive version of the tree can be found at the provided web application."), shiny::p("Click 'Zoom' to open a large version of the dendogram. Click 'Interactive' to open a new tab to the version on iTOL.", style="color: blue;")),
...
mapSettingsPanel = absolutePanel(
  ...
  dropdownButton(
    ...
    conditionalPanel("input.settings == 'MAP'",
    ...
    conditionalPanel("input.settings == 'GRAPHS'",
                     radioGroupButtons("facet", "DATA FACET",
                                       choices = c(HIV="hiv", BINDING="binding", HLA="hla"),
                                       selected = "hiv"),
                     
                     conditionalPanel("input.facet == 'hiv'",
                                      "HIV plot controls"),
                     conditionalPanel("input.facet == 'binding'",
                                      "Binding affinity plot controls"),
                     conditionalPanel("input.facet == 'hla'",
                                      "HLA plot controls")
    ),
    
    conditionalPanel("input.settings == 'DOWNLOAD'", shiny::div(
      shiny::p(shiny::h4("Data 1's title"), shinyWidgets::downloadBttn("data1", NULL, style = "simple", size = "xs"), "Description...asd iaousdb qaewjhbkac akjwsbndk jnb sau we aj a aslkjdna."),
      shiny::p(shiny::h4("Data 2's title"), shinyWidgets::downloadBttn("data2", NULL, style = "simple", size = "xs"), "Description...asd iaousdb qaewjhbkac akjwsbndk jnb sau we aj a aslkjdna."),
      shiny::p(shiny::h4("Data 3's title"), shinyWidgets::downloadBttn("data3", NULL, style = "simple", size = "xs"), "Description...asd iaousdb qaewjhbkac akjwsbndk jnb sau we aj a aslkjdna.")
    ))
  )
)